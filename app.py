...
def login():
    if request.method == 'POST':
        ...
        if doctor and check_password_hash(doctor['password'], password):
            session['doctor_id'] = doctor['id']; session['doctor_name'] = doctor['name']; session['username'] = doctor['username']
...
def admin():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""SELECT COUNT(*), COALESCE(SUM(d3_qty),0), COALESCE(SUM(magnesium_qty),0), 
                      COALESCE(SUM(CASE WHEN status='pending' THEN d3_qty END),0), 
                      COALESCE(SUM(CASE WHEN status='pending' THEN magnesium_qty END),0), 
                      COALESCE(SUM(CASE WHEN status='paid' THEN d3_qty END),0), 
                      COALESCE(SUM(CASE WHEN status='paid' THEN magnesium_qty END),0) FROM recommendations""")
    totals = cursor.fetchone()
    # Εδώ κρατάμε το αρχικό format που δούλευε
